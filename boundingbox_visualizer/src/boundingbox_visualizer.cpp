...
class BoxVisualizer{
    public:
    BoxVisualizer(): seq(0), subseq(0), last(ros::Time::now()){
        ...
        box_visual_pub = nh.advertise<jsk_recognition_msgs::BoundingBox>("box_visualizer", 1000);
        pose_sub = nh.subscribe("box_pose_topic", 1000, &BoxVisualizer::callbackBoxPose, this);

        box_arr_msg.boxes.clear();
        box_arr_msg.boxes.reserve(0);
    }
    ...
    void publishBoxMove(){
        ...
        box_msg.pose.position.x = pose_info.x + (cur - last).toSec()/10;
        box_msg.pose.position.y = pose_info.y;
        box_msg.pose.position.z = pose_info.z;
        box_msg.pose.orientation.x = 1.0;
        box_msg.pose.orientation.y = 1.0;
        box_msg.pose.orientation.z = 1.0;
        box_msg.pose.orientation.w = 1.0;
        box_msg.dimensions.x = 1.0;
        box_msg.dimensions.y = 1.0;
        box_msg.dimensions.z = 1.0;

        box_visual_pub.publish(box_msg);
    }
    private:
    ros::NodeHandle nh;
